...
abstract class Abstract_Settings {
  ...
  public function show_settings_page() {
    ...
    if (!current_user_can('manage_options')) wp_die(__('Unauthorized use.', 'kntnt-img-feeds'));
    ...
    if (isset($_POST[Plugin::ns()])) {
      ...
      if (!wp_verify_nonce($_POST['_wpnonce'], Plugin::ns())) wp_die(__('Nonce failed.', 'kntnt-img-feeds'));
  ...
  private function update_options($opt) {
    $sanitizers = wp_list_pluck($this->form(), 'sanitizer');
    foreach ($sanitizers as $field => $sanitizer) {
      $opt[$field] = $sanitizer($opt[$field]);
    }
